...
def path_finding(goal_node, door_node, available_cells):
    ...
    for i, j, heading, door_stats, key_stats, u_key in product(
        range(height),
        range(width),
        headings.keys(),
        door_status.keys(),
        ...
        new_agent_pos, new_agent_dir, key, door = motion_model(
            (headings[heading][0], headings[heading][1]),
            (i, j),
            u_key,
        ...
        if new_agent_pos == (door_node[0], door_node[1]) and door_status[door_stats] == 0:
            continue
        cost = stage_cost(new_agent_dir, new_agent_pos)
        transitions[(i, j, heading, door_stats, key_stats, u_key)] = (new_agent_pos, new_agent_dir, key, door, cost)
        
    # Value Iteration
    for _ in range(horizon):
        OLDVALUE = VALUE.copy()
        VALUE[goal_node[0], goal_node[1], :,:,:] = 0
        for (i, j, heading, door_stats, key_stats, u_key), (new_agent_pos, new_agent_dir, key, door, cost) in transitions.items():
            # Q = cost-to-go of next state + stage cost
            Q = VALUE[new_agent_pos[0], 
                      new_agent_pos[1], 
            ...
            heading_idx = headings_to_index[f'{headings[heading]}']
            key_idx = key_status[key_stats]
            door_idx = door_status[door_stats]

            if Q < VALUE[i, j, heading_idx, key_idx, door_idx] and Q != np.inf:
                VALUE[i, j, heading_idx, key_idx, door_idx] = Q
                POLICY[i, j, heading_idx, key_idx, door_idx] = actions[u_key]

        if np.all(VALUE == OLDVALUE):
            print("Value function equals")
            print("Time taken for value iteration:", time.perf_counter() - start, "seconds")
            return POLICY, VALUE
