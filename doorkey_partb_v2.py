...
def main():
    ...
    for i, j, heading, key_pos, goal_pos, door_stats, key_stats, u_key in product(
        range(height),
        range(width),
        headings.keys(),
        key_node,
        ...
        transitions[(i, j, heading, key_pos, goal_pos, door_stats, key_stats, u_key)] = (new_agent_pos, new_agent_dir, key, door, cost)
    # Value Iteration
    for _ in tqdm(range(horizon)):
        ...
        for (i, j, heading, key_pos, goal_pos, door_stats, key_stats, u_key), (new_agent_pos, new_agent_dir, key, door, cost) in transitions.items():
            ...
            h_idx = headings_to_index[f'{headings[heading]}']
            g_pos = goal_possible_positions[goal_pos]
            k_pos = key_possible_positions[key_pos]
            d_stat = door_status[door_stats]
            k_stat = key_status[key_stats]

            if Q < VALUE_T1[i, j, h_idx, g_pos, k_pos, d_stat, k_stat]:
                VALUE_T[i, j, h_idx, g_pos, k_pos, d_stat, k_stat] = Q
                POLICY[i, j, h_idx, g_pos, k_pos, d_stat, k_stat] = actions[u_key]

        if np.all(VALUE_T == VALUE_T1):
            print("Value function equals")
            print("Time taken for value iteration: ", time.perf_counter() - start)
